...
//!
//! Setting up sqlet is fairly straight forward. We
//! create a configuration with default values, a migrations
//! directory and also an initial migration (for the db type
//! chosen) that contains metadata that sqlet will use
//! for future migrations.
//!
//! This process can be adjusted by user flags. All of this
//! is mirrored in the sqlet configuration file stored
//! in `sqlet.toml` in the crate root.
//!
...
use sqlet_core::config::{Config, ConfigDbType};
use std::fs::File;
...
pub fn handle_setup() -> Result<()> {
    ...
    let mut file = File::create("./sqlet.toml").unwrap();
