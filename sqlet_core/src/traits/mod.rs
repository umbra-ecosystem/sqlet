            ...
            .any(|app| app.version() == migration.version())
        {
            if to_be_applied.contains(&migration) {
                ...
                if abort_missing_on_applied {
                    ...
                    log::warn!(target: "sqlet_core::traits::missing", "found migration on file system {} not applied", migration);
        ...
        match (current_applied, current_listing) {
            ...
            (Some(applied), Some(listing)) => {
                if abort_missing_on_applied {
                    ...
                    log::warn!(target: "sqlet_core::traits::missing", "found gap between applied migration {} and listed migration {}", applied, listing);
                }
        ...
        match migrations.iter().find(|m| m.version() == app.version()) {
            None => {
                if abort_missing_on_filesystem {
                    ...
                    log::error!(target: "sqlet_core::traits::missing", "migration {} is missing from the filesystem", app);
                }
            }
            Some(migration) => {
                if migration != app {
                    if abort_divergent {
                        ...
                        log::error!(
                            target: "sqlet_core::traits::divergent",
...
pub(crate) const DEFAULT_MIGRATION_TABLE_NAME: &str = "sqlet_schema_history";
