...
// sqlet config file used by migrate_from_config if migration from a Config struct is preferred instead of using the macros
...
mod tests {
    ...
    fn builds_db_url() {
        ...
        assert_eq!(
            "postgres://root:1234@localhost:5432/sqlet",
    ...
    #[test]
    fn builds_from_str() {
        let config = Config::from_str("postgres://root:1234@localhost:5432/sqlet").unwrap();
        assert_eq!(
            "postgres://root:1234@localhost:5432/sqlet",
            build_db_url("postgres", &config)
        );
