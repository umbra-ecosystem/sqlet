//! Contains Sqlet macros that are used to import and embed migration files.
#![recursion_limit = "128"]
...
use quote::ToTokens;
use quote::quote;
use sqlet_core::MigrationPath;
use sqlet_core::{MigrationType, find_migration_files};
use std::path::Path;
...
fn migration_enum_quoted(migration_names: &[impl AsRef<str>]) -> TokenStream2 {
    if cfg!(feature = "enums") {
        ...
        for m in migration_names {
            let m = m.as_ref();
            let (version, name) = sqlet_core::parse_migration_name(m)
...
///
/// [`Runner`]: https://docs.rs/sqlet/latest/sqlet/struct.Runner.html
#[proc_macro]
