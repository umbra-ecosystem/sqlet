...
mod mysql {
    ...
    use sqlet_core::mysql;
    use std::process::Command;
    use time::OffsetDateTime;

    const DEFAULT_TABLE_NAME: &str = "sqlet_schema_history";
    ...
    fn clean_database() {
        let opts = mysql::Opts::from_url("mysql://sqlet:root@localhost:3306/sqlet_test").unwrap();
        let mut conn = mysql::Conn::new(opts).unwrap();

        "DROP DATABASE sqlet_test".run(&mut conn).unwrap();
        "CREATE DATABASE sqlet_test".run(conn).unwrap();
    }
    ...
    fn report_contains_applied_migrations() {
        run_test(|| {
            let opts =
                mysql::Opts::from_url("mysql://sqlet:root@localhost:3306/sqlet_test").unwrap();
    ...
    //     // run_test(|| {
    //         let pool = mysql::Pool::new("mysql://sqlet:root@localhost:3306/sqlet_test").unwrap();
    //         let mut conn = pool.get_conn().unwrap();
    ...
    //         let mut query = conn
    //             .query("SELECT version FROM sqlet_schema_history")
    //             .unwrap();
    //         let row = query.next();
    //         dbg!(&row);
    //         assert!(row.is_none());
