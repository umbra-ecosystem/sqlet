...
mod mysql_async {
    ...
    use sqlet_core::mysql_async;
    use sqlet_core::mysql_async::prelude::Queryable;
    ...
    async fn clean_database() {
        let pool = mysql_async::Pool::new("mysql://sqlet:root@localhost:3306/sqlet_test");
        let mut conn = pool.get_conn().await.unwrap();

        conn.query_drop("DROP DATABASE sqlet_test").await.unwrap();
        conn.query_drop("CREATE DATABASE sqlet_test").await.unwrap();
    }

    #[tokio::test]
    async fn creates_migration_table() {
        run_test(async {
            let mut pool = mysql_async::Pool::new("mysql://sqlet:root@localhost:3306/sqlet_test");
            let mut conn = pool.get_conn().await.unwrap();
