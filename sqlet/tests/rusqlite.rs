...
mod rusqlite {
    ...
    use sqlet_core::rusqlite::Error;
    use sqlet_core::rusqlite::{Connection, OptionalExtension};
    ...
    fn doesnt_update_to_last_working_if_grouped() {
        ...
        let query: Option<u32> = conn
            .query_row("SELECT version FROM sqlet_schema_history", [], |row| {
                row.get(0)
            })
            .optional()
            .unwrap();
