...
mod tiberius {
    ...
    use sqlet_core::tiberius::{self, Config as TConfig};
    ...
    const CONFIG: &str = "mssql://SA:Passw0rd@localhost:1433/sqlet_test?trust_cert=true";
    ...
    async fn run_test<T: std::future::Future<Output = ()>>(t: T) {
        ...
        client
            .simple_query("CREATE DATABASE sqlet_test")
            .await
            .unwrap();

        client.simple_query("USE sqlet_test").await.unwrap();
        ...
        client
            .simple_query("DROP DATABASE sqlet_test")
            .await
            .unwrap();
    ...
    async fn doesnt_update_to_last_working_if_grouped() {
        run_test(async {
            ...
            let row = client
                .simple_query("SELECT version FROM sqlet_schema_history")
                .await
                .unwrap()
                .into_row()
                .await
